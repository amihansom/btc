...
#    if now.hour == 1 or now.hour == 5 or now.hour == 9 or now.hour == 13 or now.hour == 17 or now.hour == 21:
    if now.minute == 1:
        ...
        # ========== 1시간봉 거래량 받기 ==========
        price_2_close = df.iloc[ -2 ][ 'volume' ]
        price_3_close = df.iloc[ -3 ][ 'volume' ]
        price_4_close = df.iloc[ -4 ][ 'volume' ]
        price_5_close = df.iloc[ -5 ][ 'volume' ]
        price_6_close = df.iloc[ -6 ][ 'volume' ]
        price_7_close = df.iloc[ -7 ][ 'volume' ]
